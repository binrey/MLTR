...
class E2ETrain:
    ...
    def init_model(self):
        ...
        print(summary(self.model, inp_shape, device="cpu"))
    
    def load_data(self, max_size=np.Inf):
        mw = MovingWindow(DataParser(self.cfg).load()[1], self.cfg)
        self.features, self.p = next_price_prediction(mw, self.cfg.body_classifier.func, self.cfg.hist_buffer_size, max_size+1)     
    ...
    def train(self, num_epochs, resume=False, device="cpu"):
        ...
        for epoch in tqdm(list(range(num_epochs)), desc="Training"):
            optimizer.zero_grad()
            loss, _ = autoregress_sequense(self.model, p, features, epoch=epoch, device=device)
            loss -= hold[-1]
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    model.eval()
    output_seq, result_seq, fee_seq = autoregress_sequense(model, p, features, output_sequense=True, device=DEVICE)
